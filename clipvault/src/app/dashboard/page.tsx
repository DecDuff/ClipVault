...
// ✨ Explicitly define the Clip type to kill the TS errors
type Clip = typeof clips.$inferSelect;

export default async function DashboardPage() {
  let rawClips: Clip[] = []; // ✨ Type defined here
  ...
  // Sanitize for the client-side to prevent the "Digest" crash
  const allClips = rawClips.map(clip => ({
    id: String(clip.id),
    title: String(clip.title || 'Untitled'),
    videoUrl: String(clip.videoUrl || ''),
    description: String(clip.description || 'Night Drive'),
    ...
    <div className="flex min-h-screen bg-black text-white font-sans">
      ...
      <aside className="w-64 border-r border-white/5 flex flex-col p-6 sticky top-0 h-screen">
        ...
        <div className="space-y-1 pt-6 border-t border-white/5">
          ...
          <button className="w-full flex items-center gap-3 px-4 py-3 text-gray-500 hover:text-white font-bold text-sm transition-all text-xs text-left">
        ...
        <header className="flex items-center justify-between p-8">
          <div className="relative w-[450px]">
            ...
            <input 
              ...
              className="w-full bg-[#111] border border-white/5 rounded-full py-2.5 pl-12 pr-4 text-sm outline-none focus:border-white/20 transition-all text-white"
            />
          </div>
        ...
        <div className="px-8 pb-12">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex gap-2">
              <button className="px-4 py-1.5 rounded-full bg-[#111] border border-white/10 text-[10px] font-black uppercase tracking-widest text-gray-400">All Moods</button>
              <button className="px-4 py-1.5 rounded-full bg-[#111] border border-white/10 text-[10px] font-black uppercase tracking-widest text-gray-400">Cinematic</button>
            </div>
          </div>

          {allClips.length === 0 ? (
            <div className="aspect-[21/9] flex items-center justify-center border border-white/5 bg-white/[0.02] rounded-[3rem]">
              <p className="text-gray-500 font-bold uppercase tracking-widest text-xs">Vault Empty</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {allClips.map((clip) => (
                <div key={clip.id} className="group flex flex-col bg-[#0F0F0F] border border-white/5 rounded-3xl overflow-hidden transition-all hover:border-white/10">
